...
#define KECCAK_PROCESS(st, out_size, isolate)  do { \
	...
	)
{
	...
	KECCAK_PROCESS(state, 8, DAG_SIZE);
	...
	for (uint tid = 0; tid < 4; tid++)
	{
		...
		#pragma unroll 1
		for (uint a = 0; a < (ACCESSES + !DAG_SIZE); a += 8)
		{
			#pragma unroll
			for (uint y = 0; y < 8; ++y)
			{
				if (thread_id == amd_bfe(a, 3U, 2U)) {
					uint X = fnv(init0 ^ (a + y), ((uint *)&mix)[y]);
					share->uints[0] = X - (mul_hi(X, red_mult) >> red_shift) * DAG_SIZE;
				}
	...
	KECCAK_PROCESS(state, 1, DAG_SIZE);
...
static void SHA3_512(uint2 *s, uint isolate)
{
	...
	KECCAK_PROCESS(st, 8, isolate);

	for (uint i = 0; i < 8; ++i)
		s[i] = st[i];
}

__kernel void GenerateDAG(uint start, __global const uint16 *_Cache,
 __global uint16 *_DAG, uint LIGHT_SIZE)
{
	...
	SHA3_512(DAGNode.qwords, LIGHT_SIZE);
