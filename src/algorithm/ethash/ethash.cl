...
do { \
    if (get_local_id(0) == lane_idx) { \
        uint X = fnv(init0 ^ (a + x), mix.s##x); \
        buffer[hash_id] = X - (mul_hi(X, red_mult) >> red_shift) * DAG_SIZE; \
    } \
...
do { \
    uint X = fnv(init0 ^ (a + x), mix.s##x); \
    buffer[get_local_id(0)] = X - (mul_hi(X, red_mult) >> red_shift) * DAG_SIZE; \
    /*mem_fence(CLK_LOCAL_MEM_FENCE);*/ \
